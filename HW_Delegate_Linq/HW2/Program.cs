...
namespace DelegatesLinQ.Homework
{
    ...
    public class DataProcessingPipeline
    {
        ...
        public static string AddTimestamp(string input)
        {
            ...
            var current = DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss");
        ...
        public static string ReverseString(string input)
        {
            ...
            var current = new string(input.Reverse().ToArray());
            return current;
        }
        ...
        public static string ValidateInput(string input)
        {
            ...
            if (string.IsNullOrEmpty(input)) throw new ArgumentNullException("input not null or empty");
            return input;
        }
        ...
        public string ProcessData(string input, DataProcessor pipeline)
        {
            ...
            foreach (DataProcessor item in pipeline.GetInvocationList()) 
            {
                string pre = current; 
                current = item(current);
                OnProcessingStageCompleted(item.Method.Name,pre,current);   
            }
            return current; 
    ...
    public class ProcessingLogger
    {
        ...
        public void OnProcessingStageCompleted(string stage, string input, string output)
        {
            ...
            {
                logger.LogError("input not null or empty ");
            }
            logger.LogInformation($"stage: {stage} input :{input} output : {output}");
        }
        // Performance monitor class
        public class PerformanceMonitor
        {
            private Dictionary<string, TimeSpan> stageTimes = new Dictionary<string, TimeSpan>();
            ...
            public void OnProcessingStageCompleted(string stage, string input, string output)
            {
                ...
                {
                    stageTimes[stage] += sw.Elapsed;
                }
                else
                {
                    stageTimes.Add(stage, sw.Elapsed);
                }
            }
            public void DisplayStatistics()
            {
                Console.WriteLine("\t Display Performance Statistics");
                foreach (var stage in stageTimes)
                {
                    Console.WriteLine($"stageTimes : {stage.Key} - Time : {stage.Value.TotalMilliseconds} ms ");
                }
            }
        }

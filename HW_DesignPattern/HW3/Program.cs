...
namespace DesignPatterns.Homework
{
    ...
    public class WeatherStation : IWeatherStation
    {
        ...
        public void RegisterObserver(IWeatherObserver observer)
        {
            ...
            Console.WriteLine($"observer :{observer.GetType().Name} has been registered");
        }
        ...
        public void RemoveObserver(IWeatherObserver observer)
        {
            ...
            Console.WriteLine($"observer :{observer.GetType().Name} has been removed");
        ...
        {
            Console.WriteLine("observers are being notified");
    ...
    {
        private List<float> numberReading = new List<float>();
        ...
        public void Update(float temperature, float humidity, float pressure)
        {
            numberReading.Add(temperature);


        }
        public void Display()
        {
            foreach (var item in numberReading)
            {
                if (min > item)
                {
                    min = item;
            ...
            average = numberReading.Count() > 0 ? numberReading.Average() : 0;
        ...
        public class ForecastDisplay : IWeatherObserver
        {
            ...
            public void Update(float temperature, float humidity, float pressure)
            {
            if (_currentPressure == 0 && _lastPressure == 0)
            {
            ...
            public void Display()
            {
                if (_lastPressure < _currentPressure)
                {
                    Console.WriteLine("Improving weather");
                }
                else if (_lastPressure > _currentPressure)
                {
